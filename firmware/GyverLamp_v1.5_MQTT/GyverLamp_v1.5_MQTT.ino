...
/*
  ...
  - Добавлена возможность выбирать Color из RGB палитры HomeAssistant
  ...
  Версия 1.5.5
   ...
   -  Добавлено: Demo режим: в Demo режиме эффекты запускаются случайно по таймеру. Задать интервал обновления можно в переменной Timer *demoTimer = new Timer(60000);                                                                                                                                                           ^  - в миллисекундах 
   -  Добавлено: В Demo режиме пропускается эффект "Color change" если переменная epilepsy инициализирована в false
   -  Добавлено: Сохранение статуса лампы (включено/выключено)
   -  Добавлено: Доработан механизм плавного включения лампы
   -  Добавлено: Новыые эффекты - Aquarium, Starfall, Paintball!
   -  Добавлено: Переработан эффект Color change. 
   ...
   - Added: Ability to set color via http requests
...
#define DAWN_TIMEOUT 1        // сколько рассвет светит после времени Alarm clockа, минут
...
#define HEIGHT 16             // высота матрицы (вертикальные ленты)

#define COLOR_ORDER GRB       // порядок Colorов на ленте. Если Color отображается некорректно - меняйте. Начать можно с RGB
...
byte boot_count = 0;
bool demo = false;
bool epilepsy = false; // отключает эффект "Color change" в Demo режиме если задано false. 
...
Timer *demoTimer = new Timer(60000); //  время переключения эффектов в "Demo" режиме
...
void setup() {
  ...
  if ((String(MQTTConfig.HOST) == "none") || (ESP_MODE == 0) || String(MQTTConfig.HOST).length() == 0) {
    ...
    USE_MQTT = false;
    Serial.println("Using MQTT server is disabled.");
  }
